...
public class AsyncFaceTask extends AsyncTask<Void, Void, Void> implements Camera.PictureCallback {
    ...
    public AsyncFaceTask(Context context, ImageDetector imageDetector, Camera camera, boolean repeat) throws NotSupportedException {
        Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
        this.imageDetector = imageDetector;
        Camera.getCameraInfo(Camera.CameraInfo.CAMERA_FACING_FRONT, cameraInfo);
        if (!cameraInfo.canDisableShutterSound || Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) throw new NotSupportedException("Unable to disable shutter sound");
        this.camera = camera;
        if (repeat) {
            this.totalNumberOfPictures = maxRepeat;
        } else {
            this.totalNumberOfPictures = 1;
        }
